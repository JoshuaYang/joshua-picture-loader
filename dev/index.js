...
Promise.prototype.finally = function (callback) {
    const PC = this.constructor;

    return this.then(
        value => PC.resolve(callback()).then(() => value),
        reason => PC.resolve(callback()).then(() => {
            throw reason;
        })
    );
};
...
const isMobile = ua.match(/Android|webOS|iPhone|iPod|BlackBerry|IEMobile/i) !== null
